...
const DEFAULT_PHONE = '1300 906 228'
const DEFAULT_EMAIL = 'info@impactpropertymedia.com.au'

export default function ContactPage() {
  const [siteInfo, setSiteInfo] = useState({ phone: DEFAULT_PHONE, email: DEFAULT_EMAIL })
  useEffect(() => {
    fetch('/api/admin/data?table=settings').then(r => r.json()).then(data => {
      if (Array.isArray(data) && data[0]) {
        setSiteInfo({ phone: data[0].phone || DEFAULT_PHONE, email: data[0].email || DEFAULT_EMAIL })
      }
    }).catch(() => {})
  }, [])
